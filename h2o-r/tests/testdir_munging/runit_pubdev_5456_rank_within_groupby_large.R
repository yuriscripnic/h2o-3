...
test.rankwithingroupby <- function() {
  ...
  if (runif(1,0,1)>0.5) { # do test 1
    ...
    rankedF <- h2o.rank_within_group_by(train, groupCols, sortCols, sortDirs, newColName)
    h2o.summary(rankedF)
    ...
    rankedF1 <- h2o.rank_within_group_by(train, groupCols, sortCols, newColName=newColName) # optional colname
    rankedF2 <- h2o.rank_within_group_by(train, groupCols, sortCols, sortDirs) # optional directions only
    h2o.summary(rankedF)
