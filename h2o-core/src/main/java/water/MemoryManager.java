...
abstract public class MemoryManager {
  ...
  public static float  [] arrayCopyOfRange(float  [] orig, int from, int sz){ return (float []) malloc(sz,(sz-from)*8,-8,orig,from); }
  public static double [] arrayCopyOfRange(double[] orig, int from, int sz) { return (double[]) malloc(sz,(sz-from)*8,-5,orig,from); }
